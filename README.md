...
その為、下記のコードは自分が作成した物です。

- https://github.com/nekochans/lgtm-cat-frontend （Next.jsで作成したWebアプリケーションのフロントエンド部分）
- https://github.com/nekochans/lgtm-cat-ui （Reactで作成した npm package UIライブラリとして lgtm-cat-frontend から利用されます）
...
最近はフロントエンドのエンジニアリングを軸足に置いているので、少し前に開発したGoで作ったAPIのソースコードを載せておきます。

https://github.com/nekochans/portfolio-backend

また以下は [LGTMeow](https://lgtmeow.com/) でねこ画像の判定処理を作成する際に実験で作成したリポジトリです。

ちなみに小規模なサービスだと以下のようにLambdaでAPIを書く事も多いです、以下のリポジトリではデプロイに [serverless](https://www.npmjs.com/package/serverless) を使っていますが、最近ではLambda関数もTerraformで管理したほうが保守性が高いと考えているので [serverless](https://www.npmjs.com/package/serverless) はあまり利用していません。

https://github.com/keitakn/aws-rekognition-sandbox
...
最近はインフラよりもアプリケーション周り（主にフロントエンド）に関わる事が多いです。
...
最近LLMを利用したアプリケーションの新規開発案件に参加しており、学習した内容を活かしながらPythonで実務を実施しています。

また以下のようなLLMを使ったアプリケーションの個人開発も行っています。

https://zenn.dev/keitakn/scraps/49eb2616e82eb9
...
| Hono             |       A      | 個人開発で利用しました。                                            |
| FastAPI          |       A      | 個人開発、LLMを使った開発案件で利用しました。                                            |

最近の自分の考えとしては、マイクロサービスアーキテクチャが主流になってきており、
フルスタックなフレームワークを駆使してモノリシックなシステムを開発するより、
[Hono](https://hono.dev/) のような薄いフレームワークやCloudflare Workersのようなサービスを使って小規模で疎結合なシステムを構築していくのが良いと考えています。
...
Reactが好きな理由はAPIが比較的シンプルなのと、TypeScriptとの相性が良い、メンテナンスの継続性が安定しているからです。

Server Componentやアイランドアーキテクチャなどの比較的新しい分野もReact界隈から主に発信されているので、未来のフロントエンドを見据えた時にReactをキャッチアップしていたほうが得られる情報も多いというのもメリットだと思っています。
...
Next.js 13 からは React Server Components を利用可能なAppRouterが安定版になり、ますます重要な技術になったと感じています。

個人で行っている [ねこの人格を持ったAIと会話ができるWebアプリケーションの作成](https://zenn.dev/keitakn/scraps/49eb2616e82eb9) ではAppRouterを使って開発を実践しています。
以下はそのソースコードです。
...
友人と一緒に開発している個人サービス [LGTMeow](https://lgtmeow.com) でもNext.jsをVercel上で動作させているので、今後はAPI RouteをEdgeランタイムを利用した物に置き換える予定です。
...
MySQL互換のデータベースをServerlessで利用可能なサービスです。

https://planetscale.com/

Gitのbranchの概念を取り込んでおり、GitHubのようなPRを利用したデータベースSchemaの管理が可能になっていたり、安全なロールバックが可能になっていたりと非常に開発体験の良いDBになります。
